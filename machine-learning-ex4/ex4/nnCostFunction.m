...
lg = log(a3);
left = sum(sum(lg .* (-y_matrix)));

a3_prime = 1 - a3;
lg = log(a3_prime);
right = sum(sum(lg .* (1 - y_matrix)));
scaler = left - right;
...
Theta1_sum = sum(sum(Theta1_square));
Theta2_sum = sum(sum(Theta2_square));
regularized_cost = 0.5 * (lambda / m) * (Theta1_sum + Theta2_sum);
...
Theta1 = [zeros(hidden_layer_size, 1) Theta1];
